...
with tf.variable_scope('Generator'):
    G_in = tf.placeholder(dtype=tf.float32, shape=[None, N_IDEAS])
    G_l1 = tf.layers.dense(G_in, 128, tf.nn.relu)
    G_out = tf.layers.dense(G_l1, ART_COMPONENTS)

with tf.variable_scope('Descriminator'):
    real_art = tf.placeholder(dtype=tf.float32, shape=[None, ART_COMPONENTS], name='real_in')
...
for step in range(5000):
    ...
    G_paintings, pa0, Dl, _, _ = sess.run([G_out, prob_artist0, D_loss, train_D, train_G], {G_in:G_ideas, real_art:artist_paintings})
